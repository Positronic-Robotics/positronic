...
@pytest.mark.parametrize(
    'local_only', [{'timing': True}, {'output_dir': '/tmp/x'}, {'charge_inference_time': True}, {'ready_timeout': 30.0}]
)
def test_a_platform_run_refuses_what_only_a_local_run_can_mean(platform, run_command, local_only: dict):
    # The platform owns its own trial sweep, output and telemetry, and brings the policy up itself, so
    # silently dropping these would hand back a run the caller believes they configured.
