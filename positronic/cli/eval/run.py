...
def main(
    ...
    *,
    ...
    ``main`` owns the policy lifetime: it gates the run on the policy reporting itself ready and closes it
    once after the last World, so a multi-eval sweep reuses one live policy across the rebuilds.
    ...
    # Ahead of the output directory and the worlds: nothing downstream is built for a policy that cannot serve.
...
def run(
    ...
    if policy_image is None:
        ...
        main(
            ...
            timing=timing,
            ready_timeout=DEFAULT_READY_TIMEOUT if ready_timeout is None else ready_timeout,
        )
        return None
    ...
    # The platform owns its own trial sweep, output and telemetry, and brings the policy up itself.
    _refuse(
        {
            '--output-dir': output_dir,
            '--charge-inference-time': charge_inference_time,
            '--timing': timing,
