...
from platform_client.enums import NO_RESULT_STATUSES, Wire
...
def submit(
    ...
    *,
    ...
    A submission is a plan with one image endpoint on `policy_wire`: the eval names the tasks and the
    embodiment that runs them, and the catalogue expands that name. Naming one the platform does not offer
    ...
    with refusing_bad_input():
        plan = plan_of_image(
            ...
            EvalRef(eval_name),
            TypeAdapter(Slugged[Wire]).validate_python(policy_wire),
            alias=alias,
