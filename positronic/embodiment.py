...
@dataclass
class Observation:
    """A policy-facing signal source plus its two serializers.

    ``to_policy`` builds the observation entries fed to the policy; ``to_record``
    builds the entries written to the dataset. They differ today (the obs side keeps
    the raw ``RobotStatus`` that ``ErrorRecovery`` matches on); step 7 unifies them.
    ``None`` on either side passes the device value through unchanged.
    """
...
def franka(
    ...
    *,
    ...
    observations = {
        ...
        'grip': Observation(gripper.grip, None, None),
    }
    for name, emitter in (cameras or {}).items():
        observations[name] = Observation(emitter, Serializers.camera_images, Serializers.camera_images)
