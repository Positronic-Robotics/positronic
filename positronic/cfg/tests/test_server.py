...
def test_the_stage_cell_sorts_by_the_ladder_and_not_by_the_label():
    ...
    ranks = [cell[0] for cell in cells]
    labels = [cell[1] for cell in cells]
...
def test_an_episode_that_recorded_no_progress_still_sorts_against_the_ladder():
    ...
    assert cfg_server.rollout_stage_cell(EpisodeContainer({})) == cfg_server.NO_STAGE
    assert cfg_server.NO_STAGE[0] < min(stage.rank for stage in cfg_server.ProgressStage)
    assert isinstance(cfg_server.NO_STAGE[0], int)
...
def test_a_rung_the_ladder_does_not_carry_is_not_a_stage():
    ...
    assert cfg_server.rollout_stage_cell(_with_stages('teleporting')) == cfg_server.NO_STAGE
...
def test_a_stage_cell_would_spell_itself_into_a_filter_dropdown():
    ...
    would be offered as its own repr. This is what the rank the sort needs costs, recorded rather than
    discovered by whoever adds the dropdown back."""
    top = cfg_server.ProgressStage.AT_TARGET
    cell = cfg_server.rollout_stage_cell(_with_stages(top.value))
...
def test_the_model_table_counts_a_target_reached_off_the_stage_cell():
    ...
    at_target = EpisodeContainer({
        ...
        cfg_server.DERIVED_STAGE: (top.rank, top.label),
    })
