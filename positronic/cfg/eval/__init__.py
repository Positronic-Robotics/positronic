...
def build_tasks(task: Task, seed: int | None, trial_count: int, scenes: Sequence[dict] = ({},)) -> list[Task]:
    """The sweep an eval runs: one copy of ``task`` per (scene, seed) pair.

    Each ``scenes`` entry is a scene-spec base (LIBERO's suite and task id, say) swept over the seed set; the
    default single empty scene is an eval with no scene axis. ``seed`` ``None`` draws an independent random
    seed per trial; an int runs ``seed .. seed + trial_count - 1`` for every scene.
    """
