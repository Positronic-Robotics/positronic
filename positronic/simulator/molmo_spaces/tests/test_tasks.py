...
def test_timeout_defaults_to_the_benchmark_horizon_plus_a_margin(asked, caplog):
    with caplog.at_level(logging.WARNING):
        trials = list(benchmark.override(benchmark_dir='unused').instantiate().tasks())
...
def test_an_explicit_timeout_replaces_the_backstop_and_warns(asked, caplog):
    ...
    assert [short[0].timeout_sec, long[0].timeout_sec] == [20.0, 999.0]
