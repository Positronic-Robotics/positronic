...
class RobolabEnv(EnvProtocol):
    ...
    def step(self, actions: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        act[:, :7] = self._joint_targets([action[protocol.ACTION_COMMAND] for action in actions])
        # The binary gripper term closes above 0.5, so the wire grip ([0, 1], 1 = closed) feeds it as-is.
        act[:, 7] = torch.tensor(
            [float(action[protocol.ACTION_GRIP]) for action in actions], dtype=torch.float32, device=self._env.device
        )
