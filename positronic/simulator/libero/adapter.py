...
class LiberoAdapter(WireCommandAdapter):
    ...
    def terminal(self, result: dict[str, Any]) -> dict[str, Any] | None:
        # ``done`` is LIBERO's success check rather than a step limit, so reaching it is the success; a trial
        # that never does ends on the harness's budget, which records the failure.
