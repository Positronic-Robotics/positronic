...
def _check_serve(env: LiberoEnv, token: dict) -> None:
    ...
    for _ in range(5):
        out = _step(env, protocol.single_arm_action({protocol.COMMAND_TYPE: protocol.HOLD}, 0.0))
...
def _check_grip(env: LiberoEnv, token: dict) -> None:
    ...
    for _ in range(40):
        out = _step(env, protocol.single_arm_action({protocol.COMMAND_TYPE: protocol.HOLD}, 1.0))
...
def _check_obs_encoding(env: LiberoEnv, token: dict) -> None:
    ...
    _reset(env, token)
    for _ in range(40):
        wire = _step(env, protocol.single_arm_action({protocol.COMMAND_TYPE: protocol.HOLD}, 0.0))[protocol.FRAME_OBS]
    q_open = np.asarray(_robosuite_obs(env)['robot0_gripper_qpos'])
    for _ in range(40):
        wire = _step(env, protocol.single_arm_action({protocol.COMMAND_TYPE: protocol.HOLD}, 1.0))[protocol.FRAME_OBS]
    ...
    for g in np.linspace(0.0, 1.0, 6):
        for _ in range(20):
            ...
            wire = _step(env, hold)[protocol.FRAME_OBS]
