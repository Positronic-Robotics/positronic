...
@pytest.mark.parametrize(
    ...
    [
        {protocol.CMD: 'bogus'},
        {
            ...
            protocol.ACTIONS: [{protocol.ACTION_COMMAND: {protocol.COMMAND_TYPE: 'bogus'}, protocol.ACTION_GRIP: 0.0}],
        },
    ],
)
def test_server_failure_crosses_as_error_frame(env_server, message):
    ...
    assert protocol.SLOTS in conn.step([{protocol.ACTION_COMMAND: joints, protocol.ACTION_GRIP: 0.0}])
    conn.close()
