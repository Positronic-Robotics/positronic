...
def test_grasp_site_sits_at_the_2f85_grasp_point():
    ...
    transform = frame_transform(bundled_franka_model()[keys.URDF], FLANGE_LINK, GRASP_SITE_LINK)
...
def test_grasp_site_model_declares_the_frame_it_reports_in():
    ...
    assert model[keys.CONTROL_FRAME] == DEFAULT_FRAME
    transform = frame_transform(model[keys.URDF], DEFAULT_FRAME, GRASP_SITE_LINK)
