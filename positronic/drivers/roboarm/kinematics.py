"""Solving an arm's kinematics: the placo solver a URDF driver uses, and FK/IK on an MJCF model.

An arm whose controller solves Cartesian goals itself does not need either. One whose controller does not,
...
_POS_TOL = 1e-3  # meters; FK-verify acceptance for an IK solution after wrapping and clamping
...
class MjcfKinematics:
    ...
    :param reach_postures: warm starts for a target at base-frame ``(x, y)``, tried after the live posture.
        The arms this drives have a 6-DoF wrist, which gives LM no null space to escape a bad basin, so a
        seed near the goal is what makes limit-clamped IK reliable.
    """
    ...
    def __init__(
        ...
        reach_postures: Callable[[float, float], list[np.ndarray]],
    ):
    ...
    def ik(
        ...
        for start in seeds:
            ...
            if np.linalg.norm(reached.translation - target.translation) < _POS_TOL and rot_err < _ROT_TOL:
                return q
        return None
