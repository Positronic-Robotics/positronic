...
def chunk_response(actions: dict[str, np.ndarray], cot_text: str | None) -> dict:
    ...
    return {
        ACTIONS: [{name: values[i].tolist() for name, values in actions.items()} for i in range(len(arm))],
        COT_TEXT: cot_text,
    }
