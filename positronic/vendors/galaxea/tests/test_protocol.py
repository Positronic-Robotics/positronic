...
def test_chunk_transport_preserves_every_step(length):
    ...
    response = protocol.chunk_response({protocol.RIGHT_ARM: arms, protocol.RIGHT_GRIPPER: grips})
...
def test_omitted_gripper_remains_omitted_on_every_step():
    response = protocol.chunk_response({protocol.RIGHT_ARM: np.zeros((32, 7))})
...
def test_invalid_arm_chunk_fails(arms):
    with pytest.raises(ValueError):
        protocol.chunk_response({protocol.RIGHT_ARM: arms})


def test_mismatched_part_lengths_fail_instead_of_repeating_steps():
    with pytest.raises(ValueError, match='Invalid chunk'):
        protocol.chunk_response({protocol.RIGHT_ARM: np.zeros((32, 7)), protocol.RIGHT_GRIPPER: np.zeros((16, 1))})
