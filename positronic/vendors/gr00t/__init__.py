...
class ModalityConfig:
    ...
    ``state`` is positronic's own statement of what a checkpoint served under this config takes. The fork's
    config module is the other statement, and only gr00t's venv can import it, so nothing reconciles the two
    but the test pairing each config with the codec that feeds it.
    """
...
MODALITY_CONFIGS = {
    ...
    'ee': ModalityConfig('gr00t/configs/data/positronic_ee.py', _EE_QUAT),
    'ee_q': ModalityConfig('gr00t/configs/data/positronic_ee_joints.py', _EE_QUAT_JOINTS),
    # 9D xyz+rot6d configs (supports both absolute and relative actions)
    'ee_rot6d': ModalityConfig('gr00t/configs/data/positronic_ee_rot6d.py', _EE_ROT6D),
    'ee_rot6d_rel': ModalityConfig('gr00t/configs/data/positronic_ee_rot6d_rel.py', _EE_ROT6D),
    'ee_rot6d_q': ModalityConfig('gr00t/configs/data/positronic_ee_rot6d_joints.py', _EE_ROT6D_JOINTS),
    'ee_rot6d_q_rel': ModalityConfig('gr00t/configs/data/positronic_ee_rot6d_joints_rel.py', _EE_ROT6D_JOINTS),
    # Joint-space action config (for IK-derived targets)
    'joints': ModalityConfig('gr00t/configs/data/positronic_joints.py', {GRIP: 1, JOINT_POSITION: 7}),
}
