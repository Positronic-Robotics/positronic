...
def test_observation_emits_the_ordered_views_and_the_whole_state():
    ...
    # The load-time warm in `policy.py` builds its state at `STATE_DIM`, and the model takes one width.
    assert encoded[molmoact2.STATE].shape == (molmoact2.STATE_DIM,)
...
def test_observation_warms_at_the_size_the_model_tiles_to():
    ...
    assert obs[molmoact2.STATE].shape == (molmoact2.STATE_DIM,)
