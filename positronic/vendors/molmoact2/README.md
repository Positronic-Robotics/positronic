...
observations and receive decoded joint commands. `--host`, `--port`, `--uds`, `--grpc_port`, `--recording_dir`
and `--idle_timeout_min` are the server's flags. `--uds` binds the WebSocket wire to a Unix socket
path in place of `--host`/`--port`, and `--grpc_port` still binds `--host`; the model is reached through the pipeline
