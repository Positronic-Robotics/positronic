    ...
    class _Session(DelegatingSession):
        ...
        def __call__(self, obs, time_ns):
            ...
            with telemetry.span(telemetry_keys.SPAN_POLICY_STACK):
                # Unconditional, including on the ticks that skip sampling below: a gap here is a hole
                # in the window every later sample is taken from.
                self._buffer.append(now, {k: obs[k] for k in self._keys})
                stacked = self._buffer.sample(now) if self._inner.reads_observation(obs, time_ns) else None
            return self._inner(obs if stacked is None else {**obs, **stacked}, time_ns)
