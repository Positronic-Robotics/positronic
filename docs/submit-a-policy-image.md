...
5. It waits for `GET /api/v1/ready` to answer `"status": "ready"` on port 8000. VM boot, the image pull and your
...
docker exec policy /positronic/.venv/bin/python -c "import urllib.request as u; \
  print(u.urlopen(u.Request('http://127.0.0.1:8000/api/v1/ready', headers={'Authorization': 'Bearer test'})).read())"
```

The route answers `{"status": "ready", ...}` with the token once the model loads, and `401` without it.
