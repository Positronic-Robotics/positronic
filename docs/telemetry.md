...
| `episode` | `eval.pass` | `episode.index` + flat trial-context keys; at end `episode.steps`, `episode.virtual_s`; `episode.aborted` on an episode discarded rather than committed (an abort, or a world end mid-episode) | one rollout's wall |
