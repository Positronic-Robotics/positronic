...
| `robot_state.status` | int | scalar | The arm's status: `0` available, `1` resetting, `2` moving, `3` error. An arm that is faulted or resetting sends no `robot_state.ee_pose`/`q`/`dq` at all — it has no pose to give — so a stack that reads them needs `StopOnFault` ahead of it |
...
Your server receives every key each step, except that an arm which is faulted or resetting omits its `robot_state.*` measurements and sends only `robot_state.status`. The standard stack puts `StopOnFault` ahead of the model, which answers such a step itself and never forwards it; a stack without it reaches the model with those keys missing. Use what your model needs and ignore the rest. Image stream names are configuration-driven, so key off the names your deployment uses rather than assuming fixed ones. The table above is a single-arm rig; a multi-arm one names its state and grip channels per arm.
