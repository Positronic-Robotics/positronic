...
An observation codec also overrides `warm_inputs()`. It returns a zero-valued raw observation at the widths the codec reads, carrying the task it is given. The inference server encodes that through the whole server-side chain, to pay the first inference before a scored episode does ([`POST /api/v1/warm`](../positronic/offboard/README.md)). Every observation codec positronic ships overrides it; one that does not builds no warm observation, and the server warms nothing. Warm state comes from `warm_state`, and warm frames from `warm_image`.
