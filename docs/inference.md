...
**Keyboard (`real`):** press `s` to start an episode, `p` to stop and save, `q` to quit. Headless — it renders nothing — and it takes `--tasks`, `--embodiment`, `--policy` and `--output_dir`. `--tasks` names the config that makes the plan, and each press starts the next trial in it. The default plan is endless and draws a new start pose for every trial. Set the goal with `--tasks.instruction="..."`. Manual evaluation and debugging on hardware.
