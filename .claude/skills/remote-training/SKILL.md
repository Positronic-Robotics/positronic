...
uv run --locked positronic eval run --eval=.sim.positronic.stack_cubes \
  --policy=.authed_remote --policy.wire=websocket_tls --policy.address.host=<managed-host> --policy.address.port=443 \
...
```bash
uv run --locked positronic eval run --eval=.sim.positronic.stack_cubes \
  --policy=.remote --policy.address.host=desktop --policy.address.port=8000 \
  --output_dir=<...>
```
