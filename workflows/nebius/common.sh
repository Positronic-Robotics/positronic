...
# The bearer token gating served endpoints: the MysteryBox secret holding it, and the payload key inside
# that secret. Nebius requires the key to equal the environment variable the value is injected as, so it
# is AUTH_TOKEN rather than a name of ours.
