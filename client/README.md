...
never sees that address until the run records it — so a served entry is the one that may state the
`wire` it wants, `websocket` or `grpc`; stating none takes the WebSocket. An `image` one names the
